...
- `gateway/protocols.py` 继续是现有协议登记处；展示投影由 `/admin/api/protocols` 提供，前端筛选、下拉和复制目标使用同一份有效列表。
- 复制分组在多于两个协议时要求明确目标；当前仍只登记并运行 Anthropic 与 OpenAI 两种既有协议。
...
- `289d5f2`、`gateway/protocols.py`、`gateway/admin.py`、`web/util.js`、`web/views.js`：确认协议展示投影不泄露配置，复制目标不会依赖“另一个”猜测。
